...
{
    if(obj.get_columns()!=1) throw LinAlgError("Is not a vector, normalization is impossible", 1);// тут должна быть проверка линилж еррор на то что это вектор, те либо столбцы либо строки равны 1
    double norma = 0;
...
double multy_scalar(const Matrix<T1, A1>& obj1, const Matrix<T2, A2>& obj2)
{
    ...
    if(obj1.get_rows()!=obj2.get_rows()) throw LinAlgError("Number of rows is not equal, scalar multiplication is impossible", 4);
...
{
    if (int  a = obj.get_columns() != 1) throw LinAlgError("Is not a vector, cannot find unit vector", 9);// тут должна быть проверка линилж еррор на то что это вектор, те либо столбцы либо строки равны 1
